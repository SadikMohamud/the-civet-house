-- Change stamping to once per calendar day (Europe/London) and make
-- add_stamp return a status instead of raising for the expected cases
-- (already stamped today, card already complete). This lets the scanner
-- give friendly feedback rather than surfacing a database error.
...
begin
  ...
  -- One stamp per calendar day, measured in UK local time.
  select exists (
