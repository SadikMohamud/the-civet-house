...
// The stamping flow: scan a customer QR or look them up, then add a
// stamp or redeem a completed card. Shared by the staff till and the
// owner dashboard so both behave identically.
export default function TillPanel({ onStampChange }: TillPanelProps) {
  ...
  async function redeem() {
    ...
    setError(null);
    setMessage(null);
    const { data, error: err } = await getBrowserClient().rpc("redeem_reward", {
      p_customer_id: customer.customer_id,
    });
    setBusy(false);
    if (err) {
      ...
      {customer && (
        <section className="rounded-3xl bg-brand-surface p-6 shadow-sm">
          <div className="mb-4 text-center">
            <p className="font-medium">{customer.display_name || "Customer"}</p>
